...
(deftest eav-test
  (testing "Constructing and destructuring a eav."
    (let [[e a v tx-id] (eav/->EAVT 1 :todo/done false 1)]
      (is (= [1 :todo/done false 1] [e a v tx-id])))))

(defrecord MyRecord [x])

(deftest fact-type-fn-test
  (testing "Fact type of eav."
    (let [d (eav/->EAVT 1 :todo/done false 1)]
...
(deftest ancestors-fn-test
  (testing "Ancestors of keyword."
    (is (= [EAVT :eav/all] (#'eav/ancestors-fn :eav/eid))))
  #?(:clj (testing "Ancestors of my record."
            (is (some #{Associative}
                      (#'eav/ancestors-fn MyRecord))))))

(def eav-record (eav/->EAVT 1 :todo/done true 1))
(def eav-vector [1 :todo/done true 1])
...
(def saved-todo-eavs
  #{(eav/->EAVT 10 :todo/text "Buy eggs" nil)
    (eav/->EAVT 10 :todo/done true nil)})
...
(def list1 [eav-record])
(def list2 [eav-record eav-record])
